...
do   
    echo "--------------------------------------------------------------"
#     LD_PRELOAD=libtcmalloc.so OMP_NUM_THREADS=12 numactl -C 12-23 -m 5 python3 -m benchmarker --framework=torch --problem=batchmatmul \
#                         --problem_size=${m_array[i]},${n_array[i]},${k_array[i]} --batch_size=${batch_array[i]} --nb_epoch=100
	LD_PRELOAD=libtcmalloc.so OMP_NUM_THREADS=12 numactl -C 12-23 -m 5 python3 test_bmm.py ${batch_array[i]} ${m_array[i]} ${n_array[i]} ${k_array[i]}
    echo "--------------------------------------------------------------"
done
