...
int main(int argc, char * argv[]) {
    if (argc != 7)
    {
        std::cerr << "provide precision, mb, m, n, k, nb_epoch as command line parameters\n";
    ...
    dnnl::stream engine_stream(engine);
    // Tensor dimensions.
    const memory::dim MB = atoi(argv[2]), // todo(bai): a parameter should go here instead a constant
            M = atoi(argv[3]), K = atoi(argv[5]), N = atoi(argv[4]);
    ...
    std::cerr << "batch size " << MB << std::endl;
